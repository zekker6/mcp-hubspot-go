...
type fakeSearchDealsGetter struct {
	body     []byte
	err      error
	gotQuery string
	gotLimit int
...
func (f *fakeSearchDealsGetter) SearchDeals(_ context.Context, query string, limit int, properties []string, after string) ([]byte, error) {
	f.calls++
	f.gotQuery = query
	f.gotLimit = limit
...
func TestNewSearchDealsTool_Schema(t *testing.T) {
	tool := NewSearchDealsTool()
	if tool.Name != ToolNameSearchDeals {
		t.Fatalf("unexpected tool name: %q", tool.Name)
	}
	if tool.Description == "" {
		t.Fatal("expected non-empty description")
	}
	if !strings.Contains(tool.Description, "hubspot_get_active_deals") {
		t.Fatalf("expected description to cross-reference hubspot_get_active_deals, got %q", tool.Description)
	}
...
func TestSearchDealsHandler_Success(t *testing.T) {
	payload := []byte(`{"total":1,"results":[{"id":"42","properties":{"dealname":"Acme renewal"}}],"paging":{"next":{"after":"10"}}}`)
	fake := &fakeSearchDealsGetter{body: payload}
	h := SearchDealsHandler(fake)

	req := mcp.CallToolRequest{}
	req.Params.Name = ToolNameSearchDeals
	req.Params.Arguments = map[string]any{
		"query":      "acme",
		"limit":      float64(25),
		"properties": []any{"amount", "dealstage"},
		"after":      "5",
	}
	...
	if !reflect.DeepEqual(fake.gotProps, []string{"amount", "dealstage"}) {
		t.Fatalf("expected props [amount dealstage], got %v", fake.gotProps)
	}
	if fake.gotAfter != "5" {
...
func TestSearchDealsHandler_MissingQuery(t *testing.T) {
	fake := &fakeSearchDealsGetter{}
	h := SearchDealsHandler(fake)
...
func TestSearchDealsHandler_APIError(t *testing.T) {
	fake := &fakeSearchDealsGetter{err: errors.New("upstream 500")}
	h := SearchDealsHandler(fake)
...
func TestSearchDealsHandler_OptionalArgsAbsent(t *testing.T) {
	fake := &fakeSearchDealsGetter{body: []byte(`{}`)}
	h := SearchDealsHandler(fake)
	...
	if fake.gotLimit != defaultSearchDealsToolLimit {
		t.Fatalf("expected default limit=%d, got %d", defaultSearchDealsToolLimit, fake.gotLimit)
	}
...
func TestSearchDealsHandler_OptionalArgsPresent(t *testing.T) {
	fake := &fakeSearchDealsGetter{body: []byte(`{}`)}
	h := SearchDealsHandler(fake)
	...
	if !reflect.DeepEqual(fake.gotProps, []string{"amount"}) {
		t.Fatalf("expected props [amount], got %v", fake.gotProps)
	}
