...
func (f *fakeDealGetter) GetDeal(_ context.Context, id string, properties []string) ([]byte, error) {
	f.calls++
	f.gotID = id
...
func TestNewGetDealTool_Schema(t *testing.T) {
	tool := NewGetDealTool()
	if tool.Name != ToolNameGetDeal {
...
func TestGetDealHandler_Success(t *testing.T) {
	payload := []byte(`{"id":"42","properties":{"dealname":"Big Deal"}}`)
	fake := &fakeDealGetter{body: payload}
	h := GetDealHandler(fake)

	req := mcp.CallToolRequest{}
	req.Params.Name = ToolNameGetDeal
	req.Params.Arguments = map[string]any{
		"deal_id":    "42",
		"properties": []any{"amount", "pipeline"},
	}
	...
	if !reflect.DeepEqual(fake.gotProps, []string{"amount", "pipeline"}) {
		t.Fatalf("expected props [amount pipeline], got %v", fake.gotProps)
	}
...
func TestGetDealHandler_MissingDealID(t *testing.T) {
	fake := &fakeDealGetter{}
	h := GetDealHandler(fake)
...
func TestGetDealHandler_APIError(t *testing.T) {
	fake := &fakeDealGetter{err: errors.New("upstream 500")}
	h := GetDealHandler(fake)

	req := mcp.CallToolRequest{}
	req.Params.Arguments = map[string]any{"deal_id": "99"}
	res, err := h(context.Background(), req)
	if err != nil {
		t.Fatalf("handler should not error: %v", err)
	}
...
func TestGetDealHandler_PropertiesOmittedWhenAbsent(t *testing.T) {
	fake := &fakeDealGetter{body: []byte(`{}`)}
	h := GetDealHandler(fake)

	req := mcp.CallToolRequest{}
	req.Params.Arguments = map[string]any{"deal_id": "1"}
