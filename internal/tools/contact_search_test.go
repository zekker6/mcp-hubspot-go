...
type fakeSearchContactsGetter struct {
	body     []byte
	err      error
	gotQuery string
	gotLimit int
...
func (f *fakeSearchContactsGetter) SearchContacts(_ context.Context, query string, limit int, properties []string, after string) ([]byte, error) {
	f.calls++
	f.gotQuery = query
	f.gotLimit = limit
...
func TestNewSearchContactsTool_Schema(t *testing.T) {
	tool := NewSearchContactsTool()
	if tool.Name != ToolNameSearchContacts {
		t.Fatalf("unexpected tool name: %q", tool.Name)
	}
	if tool.Description == "" {
		t.Fatal("expected non-empty description")
	}
	if !strings.Contains(tool.Description, "hubspot_get_active_contacts") {
		t.Fatalf("expected description to cross-reference hubspot_get_active_contacts, got %q", tool.Description)
	}
...
func TestSearchContactsHandler_Success(t *testing.T) {
	payload := []byte(`{"total":1,"results":[{"id":"7","properties":{"email":"a@b.com"}}],"paging":{"next":{"after":"10"}}}`)
	fake := &fakeSearchContactsGetter{body: payload}
	h := SearchContactsHandler(fake)

	req := mcp.CallToolRequest{}
	req.Params.Name = ToolNameSearchContacts
	...
	if fake.gotQuery != "alice" {
		t.Fatalf("expected query=alice, got %q", fake.gotQuery)
	}
	if fake.gotLimit != 25 {
...
func TestSearchContactsHandler_APIError(t *testing.T) {
	fake := &fakeSearchContactsGetter{err: errors.New("upstream 500")}
	h := SearchContactsHandler(fake)

	req := mcp.CallToolRequest{}
	req.Params.Arguments = map[string]any{"query": "alice"}
	res, err := h(context.Background(), req)
	if err != nil {
		t.Fatalf("handler should not error: %v", err)
	}
...
func TestSearchContactsHandler_OptionalArgsAbsent(t *testing.T) {
	fake := &fakeSearchContactsGetter{body: []byte(`{}`)}
	h := SearchContactsHandler(fake)
	...
	if fake.gotLimit != defaultSearchContactsToolLimit {
		t.Fatalf("expected default limit=%d, got %d", defaultSearchContactsToolLimit, fake.gotLimit)
	}
...
func TestSearchContactsHandler_OptionalArgsPresent(t *testing.T) {
	fake := &fakeSearchContactsGetter{body: []byte(`{}`)}
	h := SearchContactsHandler(fake)
	...
	if !reflect.DeepEqual(fake.gotProps, []string{"email"}) {
		t.Fatalf("expected props [email], got %v", fake.gotProps)
	}
